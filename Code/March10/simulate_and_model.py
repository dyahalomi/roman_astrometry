...
def simulate_and_model_data(inc_earth, period_jup, roman_err, roman_duration, gaia_obs):
	...
	P_saturn = 10755.6986445	
	e_saturn = 0.0539
	Tper_saturn = 1000 #arbitrary
	omega_saturn = np.radians(338.9) - 2*np.pi
	Omega_saturn = np.radians(113.7)
	inclination_saturn = np.radians(2.486) + inclination_earth
	m_saturn = 95.159/m_sun #units m_sun 


	#add gaia observing times
	times_observed_astrometry_gaia = []
	t_gaia_0 = int(Tper_earth)
	for ii in range(t_gaia_0, t_gaia_0+3600):
	...
	times_observed_rv = []
	t_THE_0 =  times_observed_astrometry_gaia[-1] #THE will start observing around when Gaia ends
	add_data = True
	for ii in range(t_THE_0, t_THE_0+3600):
	...
	if roman_err is not None:
		t_roman_0 =  times_observed_astrometry_gaia[-1]+1800 #roman will start about 5 years after gaia ends
		times_observed_astrometry_roman = []
		for ii in range(t_roman_0, t_roman_0+(roman_duration*365)):
			if ii % 90 == 0:
	...
	if roman_err is not None:
		simulated_data.to_csv('simulated_data/period' + 
			str(int(period_jup)) + 
	...
	if roman_err is not None:
		trace.posterior.to_netcdf('./traces/period' + 
			str(int(period_jup)) + 
			'_inc' + str(int(inc_earth)) + 
			'_gaia60_roman' + 
			str(int(1e6*roman_err)) + '_' +
